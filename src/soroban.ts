...
export class SorobanReader {
  ...
  async getPolicy(corridorId: Bytes32): Promise<CorridorPolicy> {
    ...
    return {
      ...
      credentialRoot: `0x${Buffer.from(p.credential_root as Buffer).toString("hex")}` as Bytes32,
      revocationRoot: `0x${Buffer.from(p.revocation_root as Buffer).toString("hex")}` as Bytes32,
